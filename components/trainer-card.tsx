...
export function TrainerCard({ t }: { t: Trainer }) {
  return (
    <Link
      href={`/trainer/${t.id}`}
      className="group rounded-2xl overflow-hidden border border-ink-800 bg-ink-900/40 hover:border-brand-400/60 transition-all hover:-translate-y-1"
    >
      <div className="relative aspect-[5/3] overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          {t.sports.slice(0, 2).map((s) => (
            <span
              key={s}
              className="rounded-full bg-ink-950/70 backdrop-blur px-2.5 py-0.5 text-xs text-ink-100"
            >
              {s}
            </span>
          ))}
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="flex items-center gap-1 text-amber-400 font-semibold">
            ...
            {t.rating.toFixed(2)}
            <span className="text-ink-400 font-normal">({t.reviews})</span>
          </span>
        </div>
        <div className="flex items-center justify-between text-xs text-ink-400">
          ...
          <span className="flex items-center gap-1">
            {t.modality === "online" ? (
              ...
              <>
                <Video className="h-3.5 w-3.5" /> Hybrid
              </>
            )}
          </span>
