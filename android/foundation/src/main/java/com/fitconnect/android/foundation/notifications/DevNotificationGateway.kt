...
/**
 * DEBUG/TEST ONLY notification sink. Records delivered notifications for assertions.
 * Never selected by production/release composition.
 */
class DevNotificationGateway(
    ...
    override suspend fun registerForPush(): PushRegistration {
        tokenSeq += 1
        val token = "dev-fcm-token-$tokenSeq"
        logger?.i("DevNotification", "registered $token")
        return PushRegistration(token = token, provider = "dev")
    }

    override suspend fun unregisterPush() {
        logger?.i("DevNotification", "unregistered")
    }

    override suspend fun showLocal(request: LocalNotificationRequest) {
        delivered += request
        logger?.i("DevNotification", "show id=${request.id} title=${request.title}")
    }
