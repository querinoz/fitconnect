...
import { Calendar, Search, ShieldCheck, Users, Award } from "lucide-react";
  ...
  return (
    <main id="main" className="fc-marketing-hero fc-marketing-container pb-16">
      <section className="relative overflow-hidden pb-12">
        <div className="absolute inset-0 -z-10 premium-grid opacity-40" />
        <div className="mx-auto max-w-7xl px-6">
          <SectionHeader
            ...
            body={pp.subtitle}
            action={<RealtimeBadge>Programs live</RealtimeBadge>}
          />
          <div className="mt-8 grid gap-3 sm:grid-cols-3 max-w-3xl">
            <MetricTile label="Programs" value={String(PROGRAMS.length)} delta="verified coaches" tone="brand" />
            <MetricTile label="Sports" value={String(SPORTS.length)} delta="specialist paths" tone="volt" />
            <MetricTile label="Avg rating" value="4.94" delta="athlete reviews" />
          </div>
        </div>
      </section>

      <section className="mx-auto max-w-7xl px-6 py-10">
        <PremiumCard tone="brand" className="grid lg:grid-cols-2 gap-8 overflow-hidden p-0">
          ...
          <div className="p-8 lg:p-10 flex flex-col justify-center">
            ...
            {featuredCoach && (
              <div className="mt-6 flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-sm font-semibold text-ink-100">{featuredCoach.name}</p>
            ...
            {filtered.map((p) => {
              ...
              return (
                <Link
                  key={p.id}
                  href={`/programs/${p.id}`}
                  className="group rounded-2xl overflow-hidden border border-ink-800 bg-ink-900/40 hover:border-brand-400/50 transition-all hover:-translate-y-1 block"
                >
                  <div className="relative aspect-[16/9]">
                    ...
                    <div className="absolute bottom-3 left-3 right-3">
                      ...
                      <p className="text-xs text-ink-300 mt-1 line-clamp-2">{p.tagline}</p>
                    </div>
                  </div>
                  <div className="p-5 space-y-3">
                    {coach && (
                      <div className="flex items-center gap-2.5">
                        <img src={coach.avatar} alt={coach.name} className="h-8 w-8 rounded-full" />
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-ink-100 truncate">{coach.name}</p>
                          <p className="text-[11px] text-ink-500 truncate">{coach.headline}</p>
                        </div>
                      </div>
                    )}
                    ...
                    <p className="px-5 pb-5 text-xs font-semibold text-brand-300">
                      Preview program →
                    </p>
                  </div>
                </Link>
