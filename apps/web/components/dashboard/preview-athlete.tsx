...
export function PreviewAthlete({ frameless = false }: { frameless?: boolean }) {
  const body = (
      <div className={frameless ? "grid gap-3" : "grid gap-4 sm:grid-cols-3"}>
        <div className={`${frameless ? "col-span-1" : "sm:col-span-1"} rounded-2xl border border-ink-800 bg-ink-950/60 p-4 flex flex-col items-center`}>
        ...
        <div className={`${frameless ? "col-span-1" : "sm:col-span-3"} rounded-2xl border border-plasma-500/30 bg-plasma-500/5 p-4 flex items-start gap-3`}>
