...
export function CoachQuizFlow({ compact = false }: { compact?: boolean }) {
  const t = useT();
  ...
  const matches = useMemo(
    () => matchCoaches(TRAINERS, answers, 3),
    [answers]
  );
  ...
  return (
    <div className={compact ? "" : "rounded-3xl border border-ink-800 bg-ink-900/50 p-6 md:p-8 shadow-elevated"}>
      {/* Progress */}
      ...
      <div className={compact ? "min-h-[240px]" : "min-h-[280px]"}>
          ...
          {!done ? (
            ...
            >
              ...
              <div className="space-y-3">
                {matches.map(({ trainer, compatibility }) => (
                  <div
                    key={trainer.id}
                    className="flex items-center gap-4 rounded-2xl border border-ink-800 bg-ink-950/40 p-4"
                  >
                    <img
                      src={trainer.avatar}
                      alt=""
                      ...
                      height={64}
                      loading="lazy"
                      className="h-16 w-16 rounded-2xl ring-2 ring-brand-400/40 object-cover"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="shrink-0 rounded-full bg-accent-500/15 px-2 py-0.5 text-[10px] font-bold text-accent-300">
                          {compatibility}% match
                        </span>
                      </div>
                      ...
                      <p className="text-xs text-ink-500 mt-1">
                        {trainer.city}, {trainer.country} · €{trainer.hourlyRate}/h
                      </p>
                    </div>
