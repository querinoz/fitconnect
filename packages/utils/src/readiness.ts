...
export type ReadinessInputs = {
  ...
  strainScore: number;
  /** Optional 1 or 7-day history weighting (default: neutral 1.0× multiplier). */
  historyDays?: 1 | 7;
};
...
export function computeReadiness(input: ReadinessInputs): ReadinessResult {
  ...
  const sleepComponent = clamp(
    (sleepHours / 8) * 100 * (sleepEfficiency / 100),
    0,
    100
  );
  ...
  return { score, recoveryStatus, label, weights: { ...WEIGHTS } };
}
